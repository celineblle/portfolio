...
function Column({arrayKeyId} : {arrayKeyId: string}) {
  ...
  return <div className="columnComponent">
    <h2>Développeuse full stack Java, Angular, React</h2>
    <div className="columnContent">
    {arrayColumn.map((subject) => (
      <ul key={subject.id} className={openContent === subject.id ? "openColumn" : "closeColumn"} onClick={() => handleClickOpenTab(subject.id)}>
        <h3 className={openContent === subject.id ? "openColumnTitle" : "closeColumnTitle"}>{subject.name}</h3>
        <div className={openContent === subject.id ? "openCard" : "closeCard"}>
